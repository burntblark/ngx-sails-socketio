...
import {
  SailsInterceptorConstructor,
  SailsInterceptorInterface
} from "./sails.interceptor";
import {
  SailsInterceptorHandler,
  SailsInterceptorHandlerInterface
} from "./sails.interceptor.handler";
...
export class Sails
  implements SailsInterceptorInterface, SailsInterceptorHandlerInterface {
  ...
  constructor(
    ...
    socket.on(
      SailsListener.CONNECT_ERROR,
      handleListeners(SailsListener.CONNECT_ERROR)
    );
    socket.on(
      SailsListener.CONNECT_TIMEOUT,
      handleListeners(SailsListener.CONNECT_TIMEOUT)
    );
    socket.on(
      SailsListener.CONNECTING,
      handleListeners(SailsListener.CONNECTING)
    );
    socket.on(
      SailsListener.RECONNECT,
      handleListeners(SailsListener.RECONNECT)
    );
    socket.on(SailsListener.ERROR, handleListeners(SailsListener.ERROR));
    socket.on(
      SailsListener.RECONNECTING,
      handleListeners(SailsListener.RECONNECTING)
    );
    socket.on(
      SailsListener.DISCONNECT,
      handleListeners(SailsListener.DISCONNECT)
    );
  ...
  public addEventListener(
    ...
    if (!this.Listeners[eventName]) {
      throw new Error(
        `The event [${eventName}] has not yet been supported by this library.`
      );
    }
  ...
  intercept(
    request: SailsRequestOptions,
    next: SailsInterceptorHandlerInterface = this
  ): Observable<SailsResponse> {
    const handler = this.Interceptors.reduceRight(
      (next: SailsInterceptorHandlerInterface, interceptor) => {
        return new SailsInterceptorHandler(
          next,
          this.injector.get(interceptor)
        );
      },
      next
    );
  ...
  handle(request: SailsRequestOptions): Observable<SailsResponse> {
    return new Observable<SailsResponse>(obs => {
      this.socket.request(
        request.serialize(),
        (body: any, jwr: SailsIOClient.JWR.Response) => {
